...
-- Пусть этот некоторый пользователь будет, '1', 'Reuben', 'Nienow', 'arlo50@example.org', '9374071116'
-- Человек, который больше всех общался с нашим пользователем:
SELECT (SELECT id FROM users WHERE id = t1.from_user_id) AS id,
	(SELECT firstname FROM users WHERE id = t1.from_user_id) AS firstname,
    (SELECT lastname FROM users WHERE id = t1.from_user_id) AS lastname,
	number_message
FROM
	(SELECT count(*) AS number_message, from_user_id, to_user_id
    FROM messages 
	WHERE to_user_id = (SELECT id FROM users WHERE id = 1)
...
SELECT COUNT(*) AS 'Kоличество лайков младше 10 лет' FROM likes 
...
SELECT
	CASE WHEN
		...
		(SELECT COUNT(*) FROM likes WHERE user_id IN (SELECT user_id FROM `profiles` WHERE gender = 'm')) THEN 'Женщины'
		ELSE 'Мужчины'
